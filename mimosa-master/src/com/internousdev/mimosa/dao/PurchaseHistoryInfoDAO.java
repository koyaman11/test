...
public class PurchaseHistoryInfoDAO {
	...
	public List<PurchaseHistoryInfoDTO>getPurchaseHistoryList(String loginId){
		...
		String sql="select"
				...
				+ " di.family_name_kana as family_name_kana,"//姓ふりがな
				+ " di.first_name_kana as first_name_kana,"//名ふりがな
	...
	public int regist(String loginId, int productId, int productCount, int destinationId, int price){
		...
		String sql="insert into purchase_history_info(user_id, product_id, product_count, price,"
				+ " destination_id, regist_date, update_date) values (?,?,?,?,?,now(), null)";
		int count=0;
		try{
			PreparedStatement preparedStatement=connection.prepareStatement(sql);
			preparedStatement.setString(1, loginId);
